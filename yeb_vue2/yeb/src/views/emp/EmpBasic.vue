...
name: "EmpBasic",
  ...
  methods: {
    ...
    // 24-2 删除员工  ***单页删除完后，需要往上移，要判断当前有数据吗
    deleteEmp(data) {
    ...
    // 23-16 添加员工 所属部门   写不全 *这个会影像显示，下拉和workId不显示
    showDepView(){
    ...
    showAddEmpView(){
      ...
      //this.initPositions() // 23-12 获取职位  移到上面钩子中
      this.dialogVisible = true
    },
    ...
    initEmps(type){
      ...
      // this.getRequest('/employee/basic/?currentPage=').then(resp =>{
        this.loading = false;   // 9、关闭 loading
        if(resp){
          this.emps = resp.data;   // 内容
