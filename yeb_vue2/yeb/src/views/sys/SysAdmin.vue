    ...
    <div class="admin-container">
      <el-card class="admin-card" v-for="(admin,index) in admins" :key="index">
        <div slot="header" class="clearfix">
          ...
          <!-- 12、 @click="deleteAdmin(admin)" 删除 -->
          <el-button type="text" icon="el-icon-delete"  @click="deleteAdmin(admin)"
            style="float: right; padding: 3px 0;color:red;" ></el-button>
        </div>
        <div>
          <div>
...
export default {
  ...
  methods: {
    ...
    // 21、下拉框获取所有用户角色 ,没有这个，则不触发  调用initAllRoles
    showPop(admin) {
      this.initAllRoles()
