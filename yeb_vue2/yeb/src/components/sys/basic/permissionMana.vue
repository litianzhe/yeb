<template>
  <div>
    <div class="permissionManaTool">
      ...
      <el-input size="small" v-model="role.nameZh" placeholder="请输入中文名" 
                @keydown.enter.native="addRole">
      </el-input>
  ...
  methods: {
    ...
    doUpdate(rid, index) {
      ...
      this.putRequest(url).then(resp => {
        if (resp) {
          this.initSelectedMenus(rid); //刷新前端界面角色
          this.activeName = -1 // 关闭折叠面板
        }
      })
    },
    ...
    change(rid) {
      if (rid) {
        ...
        this.initSelectedMenus(rid) // 调用获取所有选中的菜单 页面显示是否勾选
